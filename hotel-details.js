...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!hotelId) {
        document.body.innerHTML = "<h2>Hotel not found. <a href='index.html'>Go Back</a></h2>";
        return;
    }
    ...
    async function loadHotelDetails(id) {
        try {
            ...
            if (data.features && data.features.length > 0) {
                ...
                document.body.innerHTML = "<h2>Hotel details unavailable. <a href='index.html'>Go Back</a></h2>";
            }
        } catch (error) {
    ...
    function renderDetails(props) {
        const name = props.name || "Hotel Details";
        ...
        container.innerHTML = `
            <div style="max-width: 800px; margin: 40px auto; padding: 20px; font-family: sans-serif;">
                ...
                <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                    ${lat && lon ? `<a href="https://www.google.com/maps/search/?api=1&query=${lat},${lon}" target="_blank" rel="noopener noreferrer" style="background: #0066cc; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold;">View on Google Maps</a>` : ""}
                    ${website ? `<a href="${escapeHTML(website)}" target="_blank" rel="noopener noreferrer" style="background: #28a745; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold;">Official Website</a>` : ""}
                </div>
            </div>
        `;
    }
